...
test-standard: tp1
	@set -e;
	@echo Archivos de prueba: entrada1 salida1 salida1ref con standard.;
	@if ./tp1 -i entrada1.txt -o salida1.txt -m standard; then\
		echo Test básico ok.;\
	else\
		echo Test fallido.;\
	fi
...
test-valgrind-standard: tp1
	...
	@echo Archivos de prueba: entrada1 salida1 salida1ref con standard.;
	@echo Probando si hubieron fugas de memoria...;
	@if valgrind --tool=memcheck --log-file="valgrind-2.txt" ./tp1 -i entrada1.txt -o salida1.txt -m standard; then\
		echo Test ok.;\
	else\
		echo Test fallido.;\
	fi
